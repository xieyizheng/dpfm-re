...
# import pyshot
def get_shot_desc(verts, faces,
                  cache_dir=None, overwrite_cache=False):
    """
    See documentation for compute_geodesic_distmat().
    This essentailly just wraps a call to compute_geodesic_distmat, using a cache if possible.
    ...
    if cache_dir:
        ...
        while True:
            ...
            try:
                ...
                # mat = torch.from_numpy(mat).to(device=device, dtype=dtype)
                shot = torch.from_numpy(shot).to(device=device, dtype=dtype)
    ...
    if not found:
        # recompute
        # mat = compute_geodesic_distmat(torch2np(verts), faces_np)
        # the returned matrix is probably np and not torch
        # max_radius = diam * options.descr_max_radius/100;
        shot = pyshot.get_descriptors(
                torch2np(verts).astype(np.float64),
                faces_np,
                radius=0.3,
                local_rf_radius=0.3,
        ...
        # save
        if cache_dir:
            # mat_np = mat.astype(dtype_np)
            shot_np = shot.astype(dtype_np)
            ...
            # mat = torch.from_numpy(mat_np).to(device=device, dtype=dtype)
            shot = torch.from_numpy(shot_np).to(device=device, dtype=dtype)

    # return mat
    return shot
def get_geodesic_distmat(verts, faces,
                  cache_dir=None, overwrite_cache=False):
    """
    See documentation for compute_geodesic_distmat().
    This essentailly just wraps a call to compute_geodesic_distmat, using a cache if possible.
